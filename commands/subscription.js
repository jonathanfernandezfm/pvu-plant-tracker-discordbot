...
module.exports = {
	...
	execute: async (msg, args, client, Discord) => {
		...
		if (userSub) {
			...
			if (args[1].includes('+')) time.add(args[1], 'days');
			else if (args[1].includes('-')) time.subtract(args[1].replace('-', ''), 'days');
			else time = moment().utc().add(args[1], 'days');
		...
		if (args[1] === 'lifetime') lifetime = true;
		else time = moment().utc().add(args[1], 'days');

		const sub = new Subscription({ user: user.id, time: time, lifetime: lifetime });
		await sub.save();
		user.roles.add('879822300193652756');
